  ...
  const AirtimeProvider: React.FC = () => {
	...
	const verifyTransactionPin = useCallback(async (email: string) => {
	  try {
		...
		if (authError || !user) {
		  throw new Error(`No authenticated user: ${authError?.message || "Unknown error"}`);
		}
	...
	//   if (!pinExists) {
	// 	Alert.alert("Action Required", "Please create a transaction PIN to proceed with your purchase.");
	// 	// Clear user inputs
	...
	// Fixed handlePurchase function with proper error handling
// const handlePurchase = async () => {
//   const { selectedProvider, phoneNumber, selectedAmount, discountedPrice } =
//     gestureRef.current;
...
//     // Fetch current wallet balance BEFORE any deduction
//     const { data: wallet, error: walletError } = await supabase
//       .from("wallet")
//       .select("balance")
//       .eq("user_email", userEmail)
...
//     const originalBalance = wallet?.balance ?? balance;

//     // Final balance check
//     if (originalBalance < discountedPrice) {
//       Alert.alert(
//         "Error",
//         `Insufficient balance. Required: ₦${formatNumberWithCommas(discountedPrice)}, Available: ₦${formatNumberWithCommas(originalBalance)}`,
//       );
//       setTransactionModalVisible(false);
//       return;
//     }

//     // DEDUCT MONEY FROM WALLET FIRST (Optimistic update)
//     const newBalance = originalBalance - discountedPrice;
...
//     if (pendingTxError) {
//       // Restore original balance on transaction record failure
//       await supabase
//         .from("wallet")
...
//     if (!purchaseResponse.ok) {
//       // API FAILED - Restore original balance (THIS IS THE FIX)
//       await supabase
//         .from("wallet")
//         .update({ balance: originalBalance }) // Restore to ORIGINAL balance
//         .eq("user_email", userEmail);
...
//       setBalance(originalBalance); // Update local state to original
//       setTransactionStatus("failed");
//       Alert.alert("Error", "Airtime purchase failed. Your wallet balance has been restored.");
//       return;
//     }
...
//     // Restore original balance on any error
//     try {
...
//       // Only restore if balance is less than expected (meaning deduction happened)
//       if (currentWallet && currentWallet.balance < balance) {
//         await supabase
//           .from("wallet")
//           .update({ balance: balance }) // Restore to the balance before purchase attempt
//           .eq("user_email", userEmail);
//         setBalance(balance);
//       }
//     } catch (restoreError) {
