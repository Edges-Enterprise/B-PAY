...
import { useAuth } from "@/context/supabase-provider"; // Adjust path accordingly
...
const ReferralPage: React.FC = () => {
  ...
  const checkEligibility = async (userId: string) => {
    ...
    const totalUserPurchases = userPurchases.reduce(
    ...
    for (const referral of referralData) {
      ...
      const totalRefereePurchases = refereePurchases.reduce(
        (sum, p) => sum + (p.amount || 0),
        0
      );
  ...
  const handleShare = async () => {
    if (!profile) return;
    try {
      const message = `Join Edges Network and get ₦200 off your first data bundle! Use my link: https://edgesnetwork.app/refer/${profile.referral_code}`; 
  ...
  const currentRank = ranks.find(
    (r) => totalReferrals >= r.min && totalReferrals <= r.max
  );
  const nextRank =
    ranks.find((r) => r.min > totalReferrals) || ranks[ranks.length - 1];
        ...
        <View style={styles.contentContainer}>
          <View style={styles.heroCard}>
            ...
            <Text style={styles.subheadline}>
              Invite 10 friends who each purchase 500GB in a month to earn 10GB free data.
              Reach the Edges rank to become an agent or reseller!
            </Text>
