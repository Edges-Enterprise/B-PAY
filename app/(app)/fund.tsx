...
const FundScreen = () => {
  ...
  const handleDeposit = async (depositAmount: number, reference: string) => {
    try {
      setIsLoading(true);
      console.log('Starting deposit:', { depositAmount, reference });
      ...
      console.log('Updating wallet:', { userEmail, currentBalance, depositAmount, profit, netAmount, newBalance });
      ...
      if (txFetchError || !txData) {
        throw new Error(`Transaction fetch failed: ${txFetchError?.message || 'No transaction'}`);
      }

      if (txData.status !== 'success') {
        console.log('Updating transaction:', { reference, id: txData.id });
      ...
      console.log('Deposit completed:', { reference, netAmount, newBalance });

      setIsLoading(false);
      Alert.alert('Success', `Wallet funded with ₦${netAmount.toLocaleString()}. Profit: ₦${profit.toLocaleString()}.`);
      setAmount('');
      router.push('/wallet');
    } catch (error) {
      console.error('handleDeposit error:', { message: error.message, reference });
      setIsLoading(false);
      Alert.alert('Error', `Deposit failed: ${error.message || 'Unknown error'}`);
      if (typeof currentBalance === 'number') {
        console.log('Reverting wallet:', { userEmail, currentBalance });
        const { error: revertError } = await supabase
          .from('wallets')
          .update({ balance: currentBalance })
          .eq('user_email', userEmail);
        if (revertError) {
          console.error('Revert wallet error:', { message: revertError.message });
        }
      }
    }
  };
...
System: It looks like your message was cut off with "throwするために" at the end, which seems incomplete. I’ll assume you intended to continue with the `FundScreen` code and address the ongoing issues: empty `verify-paystack-transaction` Edge Function logs, app logouts, the ₦1000 transaction stuck in `pending`, and wallet balance not updating. The RLS policy error (`ERROR: 42710`) has been resolved by using `CREATE OR REPLACE POLICY` in the previous response, so I’ll focus on ensuring the Edge Function executes, preventing logouts, and fixing the stuck transaction.

I’ll provide:
1. **Completed `FundScreen` Code**: Fix the `handleWebViewMessage` function and ensure session persistence.
2. **Verify Edge Function Deployment**: Ensure it’s triggered and logs appear.
3. **Fix Stuck ₦1000 Transaction**: Manual update to reflect in the wallet.
4. **Debugging Steps**: Collect logs to pinpoint why the Edge Function isn’t logging.

### Step 1: Completed `FundScreen` Code
Below is the complete `FundScreen` code with a fixed `handleWebViewMessage` function, enhanced session management, and detailed logging to trace Edge Function calls.

<xaiArtifact artifact_id="59fa1a06-f56a-4f1a-ac82-9fb4e98dc197" artifact_version_id="01cb96ea-24b6-4926-bf38-f00c4ed50e17" title="FundScreen.tsx" contentType="text/typescript">
import { useState, useEffect } from 'react';
import {
  View,
  Text,
      ...
      <View style={styles.inner}>
        ...
        >
          ...
          <View style={styles.step}>
            ...
            <Text style={styles.stepText}>Wallet credited after fees</Text>
          </View>
        </MotiView>
      </View>
    </SafeAreaView>
