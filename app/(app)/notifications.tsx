...
import { View, Text, Pressable, FlatList, StyleSheet, StatusBar, Platform } from 'react-native';
...
export default function NotificationsPage() {
    ...
    try {
      ...
      if (data.status === 'success' && Array.isArray(data.plans)) {
        const newNotifications = data.plans.map((plan: any) => ({
          ...
          metadata: { plan_name: `${preferredProvider} ${plan.data} – ₦${plan.amount}`, amount: plan.amount },
        }));
  ...
  const groupedNotifications = Object.entries(notificationGroups).map(([group, types]) => ({
    group,
